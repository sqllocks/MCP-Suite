...
const SERVER_NAME = 'mcp-error-diagnosis';
const TOOL_NAME = 'diagnose_error';
const DESCRIPTION = 'Diagnose errors and provide troubleshooting guidance';
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            if (request.params.name === TOOL_NAME) {
                return await this.handleDiagnoseError(request.params.arguments);
            }
            throw new Error(`Unknown tool: ${request.params.name}`);
        });
    }
    async handleDiagnoseError(args) {
...
const server = new McpErrorDiagnosisServer();
server.start();
